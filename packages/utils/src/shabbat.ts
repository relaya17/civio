 ...
 */
export function getShabbatGateNow(): ShabbatGateResult {
  ...
  const startHourByMonth: Record<number, number> = {
    1: 16.5,
    2: 17,
    3: 17.5,
    4: 18.5,
  ...
  const endHourByMonth: Record<number, number> = {
    1: 17.5,
    2: 17.5,
    3: 17.5,
    4: 17.5,
  ...
  const isShabbat =
    (isFriday && nowH >= shabbatStartsHour) || (isSaturday && nowH < shabbatEndsHour);
