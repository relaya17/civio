export const RIGHTS_AUTHORITIES = [
...
export const RIGHTS_ITEMS = [
    ...
    },
    {
        ...
        summary: "במצבי חוסר דיור או סיכון מיידי, נקודת הכניסה המרכזית היא לשכת הרווחה ברשות המקומית. מומלץ להגיע עם מינימום מסמכים ולבקש מענה מיידי/הפניה למסגרת.",
        steps: [
        ...
        notes: "המענים משתנים לפי עיר/מצב. המטרה כאן היא הכוונה ראשונית ותיעוד מסודר מול הגורם המטפל.",
        sources: [
            {
    ...
    },
    {
        ...
        summary: "קיימים מסלולים ייעודיים לסיוע בשכר דירה לאוכלוסיות בסיכון (כולל חסרי בית), בכפוף לתנאים ולתהליך הגשה. מומלץ להתחיל מהמידע הרשמי ולפעול עם מסמכים.",
        steps: [
...
export function getRightsAuthority(id) {
    const a = RIGHTS_AUTHORITIES.find((x) => x.id === id);
    if (!a)
        throw new Error(`Unknown rights authority: ${id}`);
    return a;
}
export function getRightsTopic(id) {
    const t = RIGHTS_TOPICS.find((x) => x.id === id);
    if (!t)
        throw new Error(`Unknown rights topic: ${id}`);
    return t;
}
